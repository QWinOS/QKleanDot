...
dotfilesrepo="https://github.com/QWinOS/QKleanDot.git"
...
usercheck() {
	! { id -u "$name" >/dev/null 2>&1; } ||
		dialog --colors --title "WARNING!" --yes-label "CONTINUE" --no-label "No wait..." --yesno "The user \`$name\` already exists on this system. Q-Script can install for a user already existing, but it will \\Zboverwrite\\Zn any conflicting settings/dotfiles on the user account.\\n\\nQ-Script will \\Zbnot\\Zn overwrite your user files, documents, videos, etc., so don't worry about that, but only click <CONTINUE> if you don't mind your settings being overwritten.\\n\\nNote also that Q-Script will change $name's password to the one you just gave." 14 70
}
...
maininstall() { # Installs all needed programs from main repo.
	dialog --title "Q-Script Installation" --infobox "Installing \`$1\` ($n of $total). $1 $2" 5 70
	installpkg "$1"
}
...
aurinstall() {
	dialog --title "Q-Script Installation" --infobox "Installing \`$1\` ($n of $total) from the AUR. $1 $2" 5 70
...
for x in curl ca-certificates base-devel git ntp fish; do
	dialog --title "Q-Script Installation" --infobox "Installing \`$x\` which is required to install and configure other programs." 5 70
	installpkg "$x"
done
...
# dialog --title "Q-Script Installation" --infobox "Installation Successful" 5 70
# yes
